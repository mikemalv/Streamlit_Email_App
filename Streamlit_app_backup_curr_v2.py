...
def handle_translation(session):
    ...
    try:
        ...
        table_query = """
            SELECT * 
            FROM CUSTOMER_SUPPORT_EMAILS 
            WHERE CORTEX_TRANSLATE IS NULL 
            OR CORTEX_TRANSLATE = ''
        """
...
def handle_summarization(session):
    ...
    try:
        ...
        table_query = """
            SELECT * 
            FROM CUSTOMER_SUPPORT_EMAILS 
            WHERE (CORTEX_SUMMARIZE IS NULL OR CORTEX_SUMMARIZE = '') 
...
def main():
    ...
    # Title and Logo
    st.image(image="https://moongolf.com/wp-content/uploads/2017/03/Callaway-logo-WHITE-1024x591-small-300x173.png")
    st.title("⛳ Support Caddy 🏌️")
